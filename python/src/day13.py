    ...
    def find_first_collision(self):
        ...
        while True:
            ...
            for cart in self.carts:
                ...
                if sum(other.position == cart.position for other in self.carts) > 1:
                    return cart.position
                # print(cart.position)
                new_location = self.map[cart.position.y][cart.position.x]
                if new_location == '+':
            ...
            for cart in self.carts:
                ...
                if new_location == '+':
                    ...
                    if cart.direction == NORTH or cart.direction == SOUTH:
                        ...
                        cart.turn_right()
            self.carts = [cart for cart in self.carts if cart.position not in crashed]
            if len(self.carts) == 1:
...
class Cart:
    ...
    def turn_right(self):
        self.direction = Cart.DIRECTIONS[
            (Cart.DIRECTIONS.index(self.direction) + 1) % len(Cart.DIRECTIONS)
        ]
